...
export const SearchResults = () => {
  ...
  const fetchPosts = async () => {
    try {
      const [resoRes, blogRes, projectRes] = await Promise.all([
      ...
      if (resoRes.data.length === 0 && blogRes.data.length === 0 && projectRes.data.length === 0) {
        setNoResults(true);
      } else {
        setNoResults(false);
      }
  ...
  return (
    <div className="sensorsCollect">
      <div className="reso-content-container">
        ...
        {searchInitiated && !noResults && (resoPosts.length > 0 || blogPosts.length > 0 || projectPosts.length > 0) && (
          <Pagination
            postsPerPage={postsPerPage}
            totalPosts={resoPosts.length + blogPosts.length + projectPosts.length}
            paginate={paginate}
          />
        )}
      </div>
